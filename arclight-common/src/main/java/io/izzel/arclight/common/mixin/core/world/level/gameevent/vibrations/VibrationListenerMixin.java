    ...
    public boolean handleGameEvent(ServerLevel worldserver, GameEvent gameevent, GameEvent.Context gameevent_a, Vec3 vec3d) {
        if (this.currentVibration != null) {
            ...
            if (optional.isEmpty()) {
                ...
                // CraftBukkit start
                boolean defaultCancel = !this.config.shouldListen(worldserver, (VibrationListener) (Object) this, new BlockPos((int) vec3d.x, (int) vec3d.y, (int) vec3d.z), gameevent, gameevent_a);
                Entity entity = gameevent_a.sourceEntity();
                BlockReceiveGameEvent event = new BlockReceiveGameEvent(org.bukkit.GameEvent.getByKey(CraftNamespacedKey.fromMinecraft(BuiltInRegistries.GAME_EVENT.getKey(gameevent))), CraftBlock.at(worldserver, new BlockPos((int) vec3d1.x, (int) vec3d1.y, (int) vec3d1.z)), (entity == null) ? null : ((EntityBridge) entity).bridge$getBukkitEntity());
